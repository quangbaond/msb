(function ($, window) {
  var urlSheet =
    "https://script.google.com/macros/s/AKfycbyYG9aFP0bAyL3gygaZ4B8KZUYsQWo6sNfkMJXBvemyPFJtSaTIRgKY0kR367SAkVfzng/exec";
  ...
  function formCalculate() {
    ...
    // hảm tính 1
    if (Number(input1) >= 500000000) {
      valueA = parseFloat(0.00015 * Number(input1) * Number(input2));

      if (valueA > 1000000) {
        valueA = 1000000 * parseInt(input2);
      }
    } else if (Number(input1) == 0) {
      valueA = 0;
    } else {
      valueA = parseFloat(12000 * Number(input2));
    }

    // tổng giao dịch của A
    totalA = parseFloat(Number(input1) * Number(input2));

    $(".tiet_kiem_trong_nuoc").text(
      valueA.toLocaleString("vi-VN", {
    ...
    valueB = parseFloat(dien_phi * rateUSD * Number(input4));

    if(valueB == 0) {
      valueB = 0;
    }else if (valueB <= rateUSD * 10) {
      valueB = rateUSD * 10;
    } else if(valueB >= rateUSD * 500) {
      valueB = rateUSD * 500 * Number(input4);
    } 

    // tổng giao dịch của B
    totalB = parseFloat(Number(input3) * Number(input4) * rateUSD);

    $(".tiet_kiem_quoc_te").text(
      valueB.toLocaleString("vi-VN", {
        minimumFractionDigits: 0,
      })
    );
    $(".totalB").text(
    ...
    var tong_tiet_kiem_chi_phi = valueA + valueB;
    ...
    // tính hoàn tiền của C
    valueC = totalA * 0.001 + totalB * 0.002;
    if (valueC > 2000000) {
      valueC = 2000000;
    }

    $(".hoan_tien").text(
      valueC.toLocaleString("vi-VN", {
        minimumFractionDigits: 0,
      })
    );
  ...
  // form thu lead
  var form_thu_lead = $("#form_thu_lead");
  var checkVali = true;
  ...
  var ten_doanh_nghiep = form_thu_lead.find('input[name="ten_doanh_nghiep"]');
  var goi_tai_khoan = form_thu_lead.find('select[name="goi_tai_khoan"]');
  var ma_cbnv = form_thu_lead.find('input[name="ma_cbnv"]');
  var i_agree = form_thu_lead.find('#i_agree');
  var doanh_thu_thue_nam_nhat = form_thu_lead.find(
    'input[name="doanh_thu_thue_nam_nhat"]'
  );
  ...
  form_thu_lead.submit(function (event) {
    ...
    var data = form_thu_lead.serialize();
    if (utmId) {
    ...
    if (checkVali && !sending) {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          form_thu_lead.find('input[type="text"]').val("");
          dia_chi.val("Hà Nội").trigger("change");
          goi_tai_khoan.val("M-Smart").trigger("change");
        },
        error: function () {
  ...
  function validateForm() {
    ...
    if (!ten_doanh_nghiep.val()) {
      toggleErrMess(ten_doanh_nghiep);
    }
  ...
  // load data form
  $(".register_smart").on("click", function () {
    goi_tai_khoan.val("M-Smart").trigger("change");
  });

  $(".register_smart_plus").on("click", function () {
    goi_tai_khoan.val("M-Smart Plus").trigger("change");
  });
  ...
  $('input[name="so_dien_thoai"], input[name="ma_so_thue"]').on(
    "input",
    function () {
